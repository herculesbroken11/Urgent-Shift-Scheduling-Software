...
-- Fix get_platform_revenue to count completed_last_minute
-- Only updating the specific subquery that filters by status = 'completed'
CREATE OR REPLACE FUNCTION public.get_platform_revenue(_months integer DEFAULT 6, _date_from text DEFAULT NULL::text, _date_to text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
          ...
          FROM (
            ...
            LEFT JOIN (
              SELECT agency_id, to_char(date_trunc('month', scheduled_start), 'YYYY-MM') as mo,
                count(*)::int cnt,
                count(*) FILTER (WHERE status IN ('completed', 'completed_last_minute'))::int completed
              FROM appointments
